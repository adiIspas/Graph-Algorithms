...
int p[100], h[100], vizitat[100], parinte[100], distanta[100];
...
int main ()
{
            ...
            if(FindSet(sortare[i][j].first) != FindSet(sortare[i][j].second))
                {
                   ...
                   // pondere_arbore[sortare[i][j].first].push_back(i);
    ...
    /*for(i = 1; i <= n; i++)
        for(j = 0; j < pondere_arbore[i].size(); j++)
            cout << i << " " << primul_arbore[i][j] << " " << pondere_arbore[i][j] << endl;*/
